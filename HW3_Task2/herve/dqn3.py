...
def train(model_class, env, pretrain=False, model= None, savepath=''):
    ...
    for init in range(0, len_config):
        ...
        x, y = config[init, 0], config[init, 1]
        print('\nAgent\'s init position : x{}, y{}'.format(x, y))
        env = construct_task2_env(x, y, dense=False)
        ...
        for episode in range(max_episodes):
            ...
            if episode % 500 == 0  and episode > 0:
                print('Test with linear init and with shaping rewards')
                mainTest(model, x, y, runs=50)
            elif episode == max_episodes-1 and init == len_config-1:
                print('Test with linear init and with shaping rewards')
                mainTest(model, x, y, runs=300)
            ...
            for t in range(t_max):
                ...
                if done and reward < 10:
                    reward = -5
                elif not done and reward < 10:
                    y_s, x_s = map(int, np.where(state[1, :] == 1))
                    y_ns, x_ns = map(int, np.where(next_state[1, :] == 1))
                    if y == 0 :
                        reward = (x_s - x_ns) / (x//2) + (y_s - y_ns)
                    else:
                        reward = (x_s - x_ns) / (x//2) + (y_s - y_ns) / y
...
def mainTest(agent, i, j, runs=25):
    ...
    def get_task(i, j, runs=300):
        tcs = [('task_2_tmax50', 50), ('task_2_tmax40', 40)]
        return {
            'time_limit': 600,
            'testcases': [{ 'id': tc, 'env': construct_task2_env(i, j), 'runs': runs, 't_max': t_max } for tc, t_max in tcs]
        }

    task = get_task(i, j, runs=runs)
