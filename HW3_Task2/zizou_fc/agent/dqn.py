...
retrain_path = os.path.join(script_path, '{}.pt'.format(model_name + '_retrained'))
history_path = os.path.join(script_path, '{}.npz'.format(model_name + '_history'))
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
    	# Mask all elements that are faw away from the agent
        agent_y, agent_x = map(int, np.where(state[1,:] == 1))
        x_front, x_back = max(0, agent_x-5), min(self.input_shape[2]-1, agent_x+5)
        y_up, y_down = max(0, agent_y-1), min(self.input_shape[1]-1, agent_y+1)

        cars, occ = np.zeros(state[0].shape), np.zeros(state[3].shape)
        cars[y_up:y_down, x_front:x_back] = state[0, y_up:y_down, x_front:x_back]
        occ[y_up:y_down, x_front:x_back] = state[3, y_up:y_down, x_front:x_back]
...
def train(model_class, env, pretrain=False, model= None):
    ...
    # Initiatlize agent's position using Curriculum learning
    # Start close to goal and get further away
...
def save_model(model, modelpath=model_path):
    '''
...
def get_env(i,j):
    ...
    '''
    return construct_task2_env(i,j, dense=True)
...
if __name__ == '__main__':
    ...
    if args.train:
      ...
      print("\nPretrain mode")
      model = get_model()
      model_p = train(ConvDQN,env, pretrain=True, model= model)
      save_model(model_p, retrain_path)
